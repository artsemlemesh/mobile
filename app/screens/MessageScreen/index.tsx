...
const MessageScreen: React.FC = ({navigation}:any) => {
  const { navigate } = navigation;
  const { user, theme } = useContext(AppContext);

  const [queryChats, { called, data, loading, error }] = useLazyQuery(
    QUERY_CHATS,
    {
  ...
  const onConnectionSelect = async (
    targetId: string,
    avatar: string,
    handle: string,
  ) => {
  ...
  const IconRight = () => (
    <IconButton
      ...
      Icon={() => (
        <Entypo name="add-to-list" size={IconSizes.x6} color={theme.text01} />
      )}
    />
  );
